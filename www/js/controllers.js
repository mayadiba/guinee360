...
.controller('AppCtrl', function($scope, $http, $localStorage, $state, $ionicHistory, $ionicSideMenuDelegate,$rootScope){
	...
	$scope.loadCategory = function(){
		if(!overCategory){
			//$http.get(wordpress_url+'/wp-json/wp/v2/categories',{
				$http.get(wordpress_url+'/wp-json/wp/v2/categories?parent='+15,{
	...
	$scope.loaded = function(){
		...
		$scope.categoryParent = $localStorage.category;
		$scope.categoryChild = {};
...
.controller('LastestsCtrl', function($scope, $http, $localStorage){
	$scope.latest = [];
	$scope.page = 1;
...
.controller('EconomyCtrl', function($scope, $http,$localStorage){
	console.log('test');
	$scope.latest = [];
	$scope.page = 1;
	$scope.load = function(isRefreshing){
		/*$http.get(wordpress_url+'/wp-json/wp/v2/posts' ,{*/
		$http.get(wordpress_url+'/wp-json/wp/v2/posts?categories='+5 ,{
			...
			angular.forEach(response.data, function(item){
				item.time = new Date(item.date_gmt).getTime();
				$scope.latest.push(item);
				//$localStorage.latests=$scope.latest;
...
.controller('PoliticsCtrl', function($scope, $http, $localStorage){
	$scope.topnews = [];;
	$scope.page = 1;
	$scope.load = function(isRefreshing){
		/*$http.get(wordpress_url+'/wp-json/wp/v2/posts?filter[orderby]=post_views&filter[order]=desc' ,{*/
		$http.get(wordpress_url+'/wp-json/wp/v2/posts?categories='+4 ,{
			...
			angular.forEach(response.data, function(item){
				...
				//$localStorage.topnews=$scope.topnews;
			});
			$scope.$broadcast('scroll.infiniteScrollComplete');
			$scope.page = $scope.page +1;
		});
...
.controller('SocietyCtrl', function($scope, $http,$localStorage){
	$scope.videos = [];
	$scope.page = 1;
	$scope.load = function(isRefreshing){
		/*$http.get(wordpress_url+'/wp-json/wp/v2/posts?filter[post_format]=post-format-video' ,{*/
		$http.get(wordpress_url+'/wp-json/wp/v2/posts?categories='+12 ,{
...
.controller('SportCtrl', function($scope, $http, $ionicScrollDelegate,$localStorage){
	$scope.trending = [];
	$scope.page = 1;
	$scope.load = function(){
		/*$http.get(wordpress_url+'/wp-json/wp/v2/posts?filter[orderby]=comment_count&filter[order]=desc' ,{*/
		$http.get(wordpress_url+'/wp-json/wp/v2/posts?categories='+6 ,{
...
.controller('DetailCtrl', function($scope, $http, $sce, $state, $stateParams, $ionicPopover, $localStorage, $ionicPopup, IonicClosePopupService, $ionicHistory){
	...
	$http.get(wordpress_url+'/wp-json/wp/v2/posts/'+$scope.posts,{ cache: true})
	.then(function(response){
...
.controller('CategoryCtrl', function($scope, $http, $sce, $stateParams, $ionicScrollDelegate,$localStorage
				){
	$scope.$sce = $sce;
$scope.category = [];
	//$scope.category = $localStorage.category;
	...
	$scope.load = function(){
		$http.get(wordpress_url+'/wp-json/wp/v2/posts?categories='+$stateParams.id, {
			...
			angular.forEach(response.data, function(item){
				...
				//$localStorage.category=$scope.category;
