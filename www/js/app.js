...
.run(function($ionicPlatform, $rootScope, $ionicLoading, $localStorage, $timeout, $http, $state, base64) {
	...
	$ionicPlatform.ready(function() {
		...
		function onOffline(){
			...
			$rootScope.$apply();
			//$scope.latest=$localStorage.latest;
		}
  	...
  	$rootScope.showLoading = function(template,$localStorage) {
		$ionicLoading.show({
		  template: template
		});
		$timeout(function(){
...
.config(function($stateProvider, $urlRouterProvider){
	...
	.state('app.forgot', {
		...
		views: {
			'menuContent': {
				templateUrl: 'templates/setting/contact.html',
...
.filter('timeAgo', function($filter){
	return function(time){
		...
		if(time_ago<0){
			...
			return Math.round(time_ago/1000) + ' secondes';
		}
		else if(time_ago<perHour){
			return Math.round(time_ago/perMinute) + ' minutes';
		}
		else if(time_ago<perDay){
			return Math.round(time_ago/perHour) + ' heures';
		}
		else if(time_ago<perMonth){
			return Math.round(time_ago/perDay) + ' jours';
		}
		else if(time_ago<perYear){
			return Math.round(time_ago/perMonth) + ' mois';
		}
		else{
			return Math.round(time_ago/perYear) + ' années';
		}
	}
})
